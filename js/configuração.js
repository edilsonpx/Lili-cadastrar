...
*/
   ...
   firebase.initializeApp(firebaseConfig);
        ...
        /***********************************************************************************/
         ...
         // REMOVER DOCUMENTOS  //
        const logoList = document.querySelector('[data-js="container-logo"]')
        db.collection('logo').get()
        .then(snapshot => {
            const logoLis = snapshot.docs.reduce((acc, doc) => {

                const {arquivoURL} = doc.data()
                acc +=` 
        ...
        logoList.addEventListener('click', e =>{
            const removeButtonId = e.target.dataset.remove
             ...
             if (removeButtonId){
                ...
                db.collection('logo').doc(removeButtonId).delete()
                .then(() =>{

                alert('logo removido com sucesso!')

                 const logo = document.querySelector(`[data-id="${removeButtonId}"]`)
              logo.remove()

                    console.log('logo removido com sucesso!')
                })
                .catch(e => {
                    console.log('Erro ao remover a logo', e.message)
                })
        ...
        tituloList.addEventListener('click', e =>{
            const removeButtonId = e.target.dataset.remove
             ...
             if (removeButtonId){
                ...
                db.collection('titulo1').doc(removeButtonId).delete()
                .then(() =>{
...
/***************************************************************************************************/
 ...
 const formTitulo01 = document.querySelector('#form-Titulo01');
        formTitulo01.addEventListener('submit', (e) => {
            e.preventDefault();
            let comprovanteText = document.querySelector('[name=comprovante01]').value;
           

            const uploadTask01 = storage.ref('titulo3/' + name).put(name);
